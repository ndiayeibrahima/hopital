...
*/
 ...
 private function create(Medecien &$medecien)
 {
 	
 	$this->pdoStatement = $this->pdo->prepare('INSERT INTO medecien VALUES(NULL, :nom, :prenom, :email, :pays , :tel)');


 	//liaison entre l'argument et la base de donnée 

 	$this->pdoStatement->bindValue(':nom', $medecien->getNom(), PDO::PARAM_STR);
 	$this->pdoStatement->bindValue(':prenom', $medecien->getPrenom(), PDO::PARAM_STR);
 	$this->pdoStatement->bindValue(':email', $medecien->getEmail(),PDO::PARAM_STR);
 $this->pdoStatement->bindValue(':pays', $medecien->getPays(), PDO::PARAM_STR);
 $this->pdoStatement->bindValue(':tel', $medecien->getTel(), PDO::PARAM_STR);
 ...
 */
 ...
 {
 	$this->pdoStatement = $this->pdo->prepare('SELECT * FROM medecien WHERE id = :id');
 	...
 	if ($executeIsOk) {
 		...
 		$medecien = $this->pdoStatement->fetchObject('Medecien');
...
{
	$this->pdoStatement = $this->pdo->query('SELECT * FROM medecien  as m , service as s , 
		specialite as sp WHERE m.idService = s.idService AND m.idSp = sp.idSp');

	// tab qui contnir les membres

	$medeciens = [];

	while($medecien = $this->pdoStatement->fetchObject('Medecien')) {
...
private function update(Medecien $medecien)
{
	$this->pdoStatement = $this->pdo->prepare('UPDATE FROM medecien SET nom=:nom,
...
$this->pdoStatement->bindValue(':id', $medecien->getId(), PDO::PARAM_INT);
...
public function delete(Medecien $medecien)
{
	$this->pdoStatement = $this->pdo->prepare('DELETE FROM medecien WHERE id=:id LIMIT 1');
...
public function save(Medecien $medecien)
{
	//si l'objet a un identifient on vas l'inserer
	//sinon on vas le mise a jour

	if (is_null($medecien->getId())) {
	
	   return $this->create($medecien);
	}
	else  

	  {
	  	return $this->update($medecien);
	  }
}
